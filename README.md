...
- SSH tunnel to WXYC MySQL (for `--generate-library-db`; or provide a pre-existing `library.db` via `--library-db`)
...
Use `--library-db /path/to/library.db` instead of `--generate-library-db` if you already have a `library.db`.

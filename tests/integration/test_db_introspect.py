...
class TestTrigramIndexesExist:
    """trigram_indexes_exist() detects GIN trigram indexes."""
...
class TestInferPipelineState:
    ...
    def test_empty_db(self, db_url) -> None:
        ...
        for step in ["create_schema", "import_csv", "create_indexes", "dedup", "prune", "vacuum"]:
    ...
    def test_after_indexes(self, db_url) -> None:
        _clean_db(db_url)
        conn = psycopg.connect(db_url, autocommit=True)
        with conn.cursor() as cur:
            cur.execute(SCHEMA_DIR.joinpath("create_database.sql").read_text())
